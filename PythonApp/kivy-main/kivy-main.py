...
class Locker:
    ...
    def lock(self, user=None):
        ...
        if token_is_equal(self.current_token, user.current_token):
            self.is_locked = True
            print(user.username + " locked Locker " + self.name)
        else:
            print("Token is invalid!")
...
class MainScreen(Screen):
    ...
    def hire_locker(self, button):
        ...
        for _ in self.lockers:
            ...
            current_locker = test_app.user.current_locker  # the locker the user is currently hiring
...
if __name__ == "__main__":
    test_app = MainApp()  # build and then run the test app
    test_app.run()
