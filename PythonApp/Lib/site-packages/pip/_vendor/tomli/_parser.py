...
def skip_until(
    ...
    *,
    ...
    if bad_chars:
        ...
        raise suffixed_err(src, bad_pos, f'Found invalid name "{bad_char!r}"')
    return new_pos
...
def parse_key_part(src: str, pos: Pos) -> Tuple[Pos, str]:
    ...
    raise suffixed_err(src, pos, "Invalid initial name for a key part")
...
def parse_basic_str_escape(
    ...
    if multiline and escape_id in {"\\ ", "\\\t", "\\\n"}:
        # Skip whitespace until next non-whitespace name or end of
...
def parse_hex_char(src: str, pos: Pos, hex_len: int) -> Tuple[Pos, str]:
    ...
    if not is_unicode_scalar_value(hex_int):
        raise suffixed_err(src, pos, "Escaped name is not a Unicode scalar value")
...
def parse_basic_str(src: str, pos: Pos, *, multiline: bool) -> Tuple[Pos, str]:
    ...
    while True:
        ...
        if char == "\\":
            ...
            continue
        if char in error_on:
            raise suffixed_err(src, pos, f'Illegal name "{char!r}"')
        pos += 1
